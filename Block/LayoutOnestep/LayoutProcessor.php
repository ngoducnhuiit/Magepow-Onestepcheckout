...
use Magepow\OnestepCheckout\Helper\Address;
...
class LayoutProcessor implements LayoutProcessorInterface
{
    ...
    public function process($jsLayout)
    {
        ...
        if (isset($jsLayout['components']['checkout']['children']['steps']['children']['shipping-step']
            ...
            if ($this->dataHelper->isEnabled()) {
                ...
                $shippingConfig['component'] = 'Magepow_OnestepCheckout/js/view/shipping-method';
                $shippingConfig['children']['customer-email']['component'] = 'Magepow_OnestepCheckout/js/view/form/element/email';
            }
        }
        ...
        foreach ($fields as $code => &$field) {
            ...
            if (!sizeof($fieldConfig)) {
                if (in_array($code, ['country_id'])) {
                    ...
                    continue;
                } else if (in_array($code, $arrayField)) {
                    unset($fields[$code]);
                }
            } else {
                $oriClasses = isset($field['config']['additionalClasses']) ? $field['config']['additionalClasses'] : '';
                $field['config']['additionalClasses'] = "{$oriClasses} col col-{$fieldConfig['colspan']}" . ($fieldConfig['isNewRow'] ? ' col-clear' : '');
     ...
     * @return $this
     */
    public function rewriteTemplate(&$field, $template = 'Magepow_OnestepCheckout/form/element/input')
    {
        if (isset($field['type']) && $field['type'] == 'group') {
            foreach ($field['children'] as $key => &$child) {
                if ($key == 0 && in_array('street', explode('.', $field['dataScope'])) && $this->dataHelper->isGoogleHttps()) {
                    $this->rewriteTemplate($child, 'Magepow_OnestepCheckout/form/element/street');
                    continue;
                }
            ...
            if ($this->dataHelper->isUsedMaterialDesign()) {
                $field['config']['template'] = 'Magepow_OnestepCheckout/form/field';
            }
        }
    ...
    public function rewriteFieldStreet(&$fields)
    {
        ...
        if ($this->dataHelper->isUsedMaterialDesign()) {
            $fields['country_id']['config']['template'] = 'Magepow_OnestepCheckout/form/field';
            $fields['region_id']['config']['template'] = 'Magepow_OnestepCheckout/form/field';
            foreach ($fields['street']['children'] as $key => $value) {
                $fields['street']['children'][0]['label'] = $fields['street']['label'];
                $fields['street']['children'][$key]['config']['template'] = 'Magepow_OnestepCheckout/form/field';
            }
            $fields['street']['config']['fieldTemplate'] = 'Magepow_OnestepCheckout/form/field';
    ...
    public function getCustomBillingAddressComponent($elements)
    {
        ...
        $fields = [
            'component' => 'Magepow_OnestepCheckout/js/view/billing-address',
